...
import 'package:discuss_hub_project/core/providers/storage_repository_provider.dart';
...
import 'package:discuss_hub_project/features/community/repository/communitory_repository.dart';
...
final userCommunitiesProvider = StreamProvider((ref) {
  ...
  if (user == null || !user.isAuthenticated) {
    return Stream.value([]); // guest users shouldn't see anything
  }
...
final exploreCommunitiesProvider = StreamProvider((ref) {
  final communityController = ref.watch(communityControllerProvider.notifier);
  final user = ref.watch(userProvider)!;
  return communityController.getExploreCommunities(user.uid);
});
...
class CommunityController extends StateNotifier<bool> {
  ...
  Stream<List<Community>> getExploreCommunities(String uid) {
    return _communityRepository.getAllCommunities().map(
      (communities) {
        return communities.where((community) {
          return !community.members.contains(uid) &&
